...
import { Refrigerator, Plus, Trash2, RefreshCw } from 'lucide-react';
...
function CostForm({
  ...
  const destroySource = useSourcesDestroy({
    mutation: { onSuccess: () => { setDeleteConfirmId(null); invalidate(); } },
  });
      ...
      <div className="space-y-2">
        ...
        {sources.length === 0 ? (
          ...
          <ul className="space-y-2">
            {sources.map(src => (
              <li key={src.id} className="text-sm border border-palette-border rounded-md p-2 flex items-center gap-2">
                {deleteConfirmId === src.id ? (
                  ...
                  <>
                    ...
                    <button
                      type="button"
                      className="text-palette-textMuted hover:text-red-500 shrink-0 p-1"
